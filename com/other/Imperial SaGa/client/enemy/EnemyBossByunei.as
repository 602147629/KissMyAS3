package enemy
{
    ...
    public class EnemyBossByunei extends EnemyDisplay
    {
        private var _WAIT_EFFECT:Number = 2;
        private var _effectManager:EffectManager;
        private var _effectWaitTime:Number;
        private static const _EFFECT_PATH:String = ResourcePath.ENEMY_EFFECT_SWF_PATH + "Effect_Byunei_EF00.swf";

        public function EnemyBossByunei(param1:DisplayObjectContainer, param2:int, param3:int)
        {
        ...
        private function playSeMotion() : void
        {
            ...
            {
                SoundManager.getInstance().playSe(SoundId.SE_REV_BUNE_ATTACK_START);
            }
            if (_labelAnimDetail == "se1002")
            {
                SoundManager.getInstance().playSe(SoundId.SE_REV_BUNE_ATTACK_SWORDSET);
            }
            if (_labelAnimDetail == "se1003")
            {
                SoundManager.getInstance().playSe(SoundId.SE_REV_BUNE_ATTACK_SWISH);
            }
            ...
            {
                SoundManager.getInstance().playSe(SoundId.SE_REV_BUNE_MAGIC_TAME);
            }
            return;
        }// end function
        ...
        {
            return [SoundId.SE_REV_BUNE_ATTACK_START, SoundId.SE_REV_BUNE_ATTACK_SWORDSET, SoundId.SE_REV_BUNE_ATTACK_SWISH, SoundId.SE_REV_BUNE_ATTACK_START, SoundId.SE_REV_BUNE_MAGIC_TAME];
        }// end function
